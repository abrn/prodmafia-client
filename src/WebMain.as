    ...
    public class WebMain extends Sprite {
        ...
        private function createContext(): void {
            ...
            this.context.extend(MVCSBundle, SignalCommandMapExtension)
                    .configure(BuildConfig, StartupConfig, NetConfig, AssetsConfig,
                            DialogsConfig, ApplicationConfig, LanguageConfig,
                            TextConfig, AppEngineConfig, AccountConfig, ErrorConfig, CoreConfig,
                            ...
                            TooltipsConfig, PromotionsConfig, ProTipConfig, ClassesConfig,
                            PackageConfig, PetsConfig, DailyLoginConfig, Stage3DConfig, ArenaConfig,
                            ExternalConfig, MysteryBoxConfig, DailyQuestsConfig, SocialConfig,
                            NexusShopConfig, ToSConfig, SupportCampaignConfig, SeasonalEventConfig, this);
        ...
        public static function uncaughtErrorHandler(event: UncaughtErrorEvent): void {
            ...
            if (event.error is Error) {
                errorText = (event.error as Error).getStackTrace();
            ...
            var errorLog: File = File.documentsDirectory.resolvePath("prod-errors.log");
            var _loc5_: FileStream = new FileStream();
            _loc5_.open(errorLog, "append");
            var time: Date = new Date();
            _loc5_.writeUTFBytes("\n[" + (time.monthUTC + 1) + "/" + time.dateUTC + "/" + time.fullYearUTC + "" + trailZero(time.hoursUTC) + ":" + trailZero(time.minutesUTC) + ":" + trailZero(time.secondsUTC) + "] " + errorText);
            _loc5_.close();
