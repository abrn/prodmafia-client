...
public class SavedCharactersList extends Event {
    ...
    private function updateAccount():void {
        ...
        while (_local2 < ObjectLibrary.playerChars_.length) {
            _local1 = ObjectLibrary.playerChars_[_local2];
            _local3 = _local1.@type;
            if (this.levelRequirementsMet(_local3)) {
                this.account.reportIntStat(_local1.@id + "Unlocked", 1);
                _local4++;
            }
            _local2++;
        }
        this.account.reportIntStat("ClassesUnlocked", _local4);
    }
}
}
