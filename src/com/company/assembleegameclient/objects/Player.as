package com.company.assembleegameclient.objects {
    ...
    public class Player extends Character {
        ...
        public static function fromPlayerXML(username: String, data: XML): Player {
            var playerObj: * = null;
            var currentPlayer: Player = null;
            var playerName: * = username;
            var playerXML: * = data;
            var objType: int = playerXML.ObjectType;
            try {
                playerObj = ObjectLibrary.xmlLibrary_[objType];
                currentPlayer = new Player(playerObj);
                currentPlayer.name_ = playerName;
                currentPlayer.level_ = playerXML.Level;
                currentPlayer.exp_ = playerXML.Exp;
                currentPlayer.equipment_ = ConversionUtil.toIntVector(playerXML.Equipment);
                currentPlayer.calculateStatBoosts();
                currentPlayer.lockedSlot = new Vector.<int>(currentPlayer.equipment_.length);
                currentPlayer.maxHP_ = currentPlayer.maxHPBoost_ + int(playerXML.MaxHitPoints);
                currentPlayer.hp_ = int(playerXML.HitPoints);
                currentPlayer.maxMP_ = currentPlayer.maxMPBoost_ + int(playerXML.MaxMagicPoints);
                currentPlayer.mp_ = int(playerXML.MagicPoints);
                currentPlayer.attack_ = currentPlayer.attackBoost_ + int(playerXML.Attack);
                currentPlayer.defense_ = currentPlayer.defenseBoost_ + int(playerXML.Defense);
                currentPlayer.speed_ = currentPlayer.speedBoost_ + int(playerXML.Speed);
                currentPlayer.dexterity_ = currentPlayer.dexterityBoost_ + int(playerXML.Dexterity);
                currentPlayer.vitality_ = currentPlayer.vitalityBoost_ + int(playerXML.HpRegen);
                currentPlayer.wisdom = currentPlayer.wisdomBoost_ + int(playerXML.MpRegen);
                currentPlayer.tex1Id_ = !!playerXML.hasOwnProperty("Tex1") ? int(playerXML.Tex1) : 0;
                currentPlayer.tex2Id_ = !!playerXML.hasOwnProperty("Tex2") ? int(playerXML.Tex2) : 0;
                currentPlayer.hasBackpack_ = playerXML.HasBackpack == "1";
            } catch (error: Error) {
                throw new Error("Type: 0x" + objType.toString(16) + " doesn\'t exist. " + error.message);
            }
            return currentPlayer;
        }
        
        public function Player(data: XML) {
            ...
            var stage: Injector = StaticInjectorContext.getInjector();
            this.addTextLine = stage.getInstance(AddTextLineSignal);
            this.factory = stage.getInstance(CharacterFactory);
            this.supportCampaignModel = stage.getInstance(SupporterCampaignModel);
            super(data);
            
            this.attackMax_ = int(data.Attack.@max);
            this.defenseMax_ = int(data.Defense.@max);
            this.speedMax_ = int(data.Speed.@max);
            this.dexterityMax_ = int(data.Dexterity.@max);
            this.vitalityMax_ = int(data.HpRegen.@max);
            this.wisdomMax_ = int(data.MpRegen.@max);
            this.maxHPMax_ = int(data.MaxHitPoints.@max);
            this.maxMPMax_ = int(data.MaxMagicPoints.@max);
        ...
        public var exaltationICReduction: int = 0;
        ...
        override public function update(param1: int, param2: int): Boolean {
            ...
            if (this.relMoveVec_) {
                ...
                if (this.rotate_ != 0) {
                    //todo: add parameter for camera rotate speed
        ...
        override protected function makeNameBitmapData(): BitmapData {
            var data: BitmapData = BitmapTextFactory.make(!!name_ ? name_ : className, 16, this.getNameColor(), true, NAME_OFFSET_MATRIX, true);
            data.draw(FameUtil.numStarsToIcon(this.numStars_, this.starsBg_), RANK_STAR_BG_OFFSET_MATRIX);
            return data;
        }
        
        override public function draw(elements: Vector.<GraphicsBitmapFill>, camera: Camera, time: int): void {
        ...
        override protected function getTexture(camera: Camera, time: int): BitmapData {
            ...
            if (camera.isHallucinating_) {
                var objTexture = new MaskedImage(getHallucinatingTexture(), null);
            } else {
                var objTexture = animatedChar_.imageFromFacing(facing_, camera, _loc11_, _loc10_);
            }
            
            var fakeText1: int = tex1Id_;
            var fakeText2: int = tex2Id_;
            if (fakeTex1 != -1) {
                fakeText1 = fakeTex1;
            }
            if (fakeTex2 != -1) {
            ...
            if (this.nearestMerchant_) {
                nearestMerchant = texturingCache_[this.nearestMerchant_];
                if (nearestMerchant == null) {
                    texturingCache_[this.nearestMerchant_] = new Dictionary();
                } else {
                    _loc13_ = nearestMerchant[objTexture];
                }
                fakeText1 = this.nearestMerchant_.getTex1Id(tex1Id_);
                fakeText2 = this.nearestMerchant_.getTex2Id(tex2Id_);
            } else {
                _loc13_ = texturingCache_[objTexture];
            }
            if (_loc13_ == null) {
                _loc13_ = TextureRedrawer.resize(objTexture.image_, objTexture.mask_, size_, false, fakeText1, fakeText2);
                if (this.nearestMerchant_ != null) {
                    texturingCache_[this.nearestMerchant_][objTexture] = _loc13_;
                } else {
                    texturingCache_[objTexture] = _loc13_;
                }
            }
            if (hp_ < maxHP_ * 0.2) {
                colorOffset = Math.abs(Math.sin(time * 0.005)) * 10 * 0.1;
               var colorTransform = new ColorTransform(1, 1, 1, 1, colorOffset * 128, -colorOffset * 128, -colorOffset * 128);
                _loc13_ = CachingColorTransformer.transformBitmapData(_loc13_, colorTransform);
            }
            var bitmapData: BitmapData = texturingCache_[_loc13_];
            if (bitmapData == null) {
                if (this == this.map_.player_) {
                    if (Parameters.VHS == 1) {
                        bitmapData = GlowRedrawer.outlineGlow(_loc13_, 65280);
                    } else if (Parameters.VHS == 2) {
                        bitmapData = GlowRedrawer.outlineGlow(_loc13_, 16768256);
                    } else if (this.hasSupporterFeature(1)) {
                        bitmapData = GlowRedrawer.outlineGlow(_loc13_, 13395711, 1.4, false, true);
                    } else {
                        bitmapData = GlowRedrawer.outlineGlow(_loc13_, this.legendaryRank_ == -1 ? 0 : 16711680);
                    }
                } else if (this.hasSupporterFeature(1)) {
                    bitmapData = GlowRedrawer.outlineGlow(_loc13_, 13395711, 1.4, false, true);
                } else {
                ...
                texturingCache_[_loc13_] = bitmapData;
            }
            if (Parameters.data.alphaOnOthers && (this.objectId_ != map_.player_.objectId_ && (!this.starred_ || this.isFellowGuild_ && Parameters.data.showAOGuildies))) {
                bitmapData = CachingColorTransformer.alphaBitmapData(bitmapData, Parameters.data.alphaMan);
            } else if (this.isPaused || this.isStasis || this.isPetrified) {
                bitmapData = CachingColorTransformer.filterBitmapData(bitmapData, PAUSED_FILTER);
            } else if (this.isInvisible) {
                bitmapData = CachingColorTransformer.alphaBitmapData(bitmapData, 0.4);
            }
            return bitmapData;
        }
        ...
        override public function getPortrait(): BitmapData {
            if (portrait_ == null) {
                var sprite: * = animatedChar_.imageFromDir(0, 0, 0);
                var height: int = 4 / sprite.image_.width * 100;
                portrait_ = TextureRedrawer.resize(sprite.image_, sprite.mask_, height, true, tex1Id_, tex2Id_);
                portrait_ = GlowRedrawer.outlineGlow(portrait_, 0);
            }
            return portrait_;
        }
        
        override public function setAttack(objectId: int, newAttack: Number): void {
            var object: XML = ObjectLibrary.xmlLibrary_[objectId];
            if (object == null || !("RateOfFire" in object)) {
                return;
            }
            var oldRoF: Number = object.RateOfFire;
            this.attackPeriod_ = 1 / this.attackFrequency() * (1 / oldRoF);
            super.setAttack(objectId, newAttack);
        }
        
        override public function removeFromMap(): void {
            if (Parameters.followingName && Parameters.data.followIntoPortals && this.name_.toUpperCase() == Parameters.followName) {
                var obj: int = 0;
                var mapObjects: * = map_.goDict_;
                for each(var obj in mapObjects) {
                    if (obj is Portal && this.getDistSquared(x_, y_, obj.x_, obj.y_) <= 1) {
                        this.map_.gs_.gsc_.usePortal(obj.objectId_);
                        break;
                    }
                }
            }
        ...
        public function combatTrigger(): int {
            ...
            var playerDef: int = this.defense_;
            var _loc4_: int = Math.floor(playerDef / 15);
            counter = 0;
            while (counter < _loc4_) {
                returnValue = returnValue + 15 * _loc5_[Math.min(counter, 3)];
                counter++;
            }
            returnValue = returnValue + playerDef % 15 * _loc5_[Math.min(Math.max(0, _loc4_ - 1), 3)];
            return returnValue;
        }
        
        public function icTime(): Number {
            return Math.max(7000 - this.vitality_ * 50, 1000);
        }
        ...
        public function calculateStatBoosts(): void {
            var itemBoosts: * = undefined;
            var currentItemIndex: int = 0;
            var object: * = null;
            var equipment: * = null;
            var incrType: int = 0;
            var incrTypeId: * = undefined;
            var incrAmount: int = 0;
            var counter: int = 0;
            ...
            while (counter < 4) {
                if (equipment_ && equipment_.length > counter) {
                    currentItemIndex = equipment_[counter];
                    
                    if (currentItemIndex != -1) {
                        object = ObjectLibrary.xmlLibrary_[currentItemIndex];
                        
                        if (object != null && object.hasOwnProperty("ActivateOnEquip")) {
                            itemBoosts = object.ActivateOnEquip;
                            
                            for each(equipment in object.ActivateOnEquip) {
                                if (equipment.toString() == "IncrementStat") {
                                    incrType = equipment.@stat;
                                    incrAmount = equipment.@amount;
                                    incrTypeId = incrType;
                                    var switchType: * = incrTypeId;
                                    switch (switchType) {
                                    case 0:
                                        this.maxHPBoost_ = this.maxHPBoost_ + incrAmount;
                                        continue;
                                    case 3:
                                        this.maxMPBoost_ = this.maxMPBoost_ + incrAmount;
                                        continue;
                                    case 20:
                                        this.attackBoost_ = this.attackBoost_ + incrAmount;
                                        continue;
                                    case 21:
                                        this.defenseBoost_ = this.defenseBoost_ + incrAmount;
                                        continue;
                                    case 22:
                                        this.speedBoost_ = this.speedBoost_ + incrAmount;
                                        continue;
                                    case 26:
                                        this.vitalityBoost_ = this.vitalityBoost_ + incrAmount;
                                        continue;
                                    case 27:
                                        this.wisdomBoost_ = this.wisdomBoost_ + incrAmount;
                                        continue;
                                    case 28:
                                        this.dexterityBoost_ = this.dexterityBoost_ + incrAmount;
                                    default:
                                        continue;
                                    }
                                }
        ...
        public function setRelativeMovement(rotate: Number, posX: Number, poxY: Number): void {
            this.rotate_ = rotate;
            
            if (!this.relMoveVec_) {
                this.relMoveVec_ = new Point();
            }
            this.relMoveVec_.x = posX;
            this.relMoveVec_.y = poxY;
    
            var currentX: Number = NaN;
            if (this.isConfused) {
                currentX = this.relMoveVec_.x;
                this.relMoveVec_.x = -this.relMoveVec_.y;
                this.relMoveVec_.y = -currentX;
        ...
        public function hasSupporterFeature(flag: int): Boolean {
            return (this.supporterFlag & flag) == flag;
        }
        
        public function setTokens(amount: int): void {
            this.tokens_ = amount;
        }
        ...
        public function setGuildName(guildName: String): void {
            ...
            var currentPlayer: Player = map_.player_;
            if (currentPlayer == this) {
                for each(player in map_.goDict_) {
                    playerObject = player as Player;
                    if (playerObject != null && playerObject != this) {
                        playerObject.setGuildName(playerObject.guildName_);
                    }
                }
            } else {
                duplicate = currentPlayer && currentPlayer.guildName_ && currentPlayer.guildName_ != "" && currentPlayer.guildName_ == this.guildName_;
                
                if (duplicate != this.isFellowGuild_) {
                    this.isFellowGuild_ = duplicate;
        ...
        public function isTeleportEligible(currentPlayer: Player): Boolean {
            return !(currentPlayer.dead_ || currentPlayer.isPaused || currentPlayer.isInvisible);
        }
        
        public function msUtilTeleport(): int {
            var currentTime: int = TimeUtil.getTrueTime();
            return Math.max(0, this.nextTeleportAt_ - currentTime);
        }
        
        public function teleportTo(target: Player): Boolean {
            map_.gs_.gsc_.teleport(target.objectId_);
            return true;
        }
        
        public function levelUpEffect(messageText: String, particles: Boolean = true): void {
            if (particles && !Parameters.data.noParticlesMaster) {
                this.levelUpParticleEffect();
            }
            var text: CharacterStatusText = new CharacterStatusText(this, 65280, 2000);
            text.setText(messageText);
            map_.mapOverlay_.addStatusText(text);
        }
        
        public function handleLevelUp(unlock: Boolean): void {
        ...
        public function handleExpUp(xpAmount: int): void {
            if (level_ == 20 && !bForceExp()) {
                return;
            }
            var text: CharacterStatusText = new CharacterStatusText(this, 65280, 1000);
            text.setText("+" + xpAmount + " EXP");
            map_.mapOverlay_.addStatusText(text);
        }
        
        public function updateFame(newFame: int): void {
            var text: CharacterStatusText = new CharacterStatusText(this, 14835456, 2000);
            text.setText("+" + newFame + " Fame");
            map_.mapOverlay_.addStatusText(text);
        }
        ...
        public function attemptAutoAbility(param1: Number, param2: int = -1, param3: int = 0): void {
            ...
            if (param3 != -1 && Parameters.data.AutoAbilityOn && !this.map_.gs_.isSafeMap && Parameters.abi && !Parameters.data.fameBlockAbility && this.mp_ >= this.autoMpPercentNumber) {
        ...
        public function shootAutoAimWeaponAngle(param1: int, param2: int): Boolean {
            ...
            _loc5_ = _loc8_.speed_ * this.projectileSpeedMult * (_loc8_.lifetime_ * this.projectileLifeMult);
            _loc6_ = this.calcAimAngle(_loc8_.speed_ * this.projectileSpeedMult, _loc5_ + Parameters.data.aaDistance, _loc10_, _loc4_);
            if (_loc6_) {
        ...
        public function pickup(param1: GameObject, param2: int, param3: int): void {
            var _loc5_: int = 0;
            var _loc4_: Vector.<int> = new <int>[2736, 16874, 1799, 2594, 2868, 2870, 2872, 2874, 2876, 2836, 2837, 2838, 2839, 2689, 2632, 2633, 2795, 1799, 3105, 3090, 3164, 3163, 3265, 9077, 10244, 10243, 28901];
            if (_loc4_.indexOf(param3) != -1) {
                if (this.healthPotionCount_ == 6 && param3 == 2594) {
                    if (Parameters.data.autoLootHPPotsInv) {
                        _loc5_ = findItem(this.equipment_, -1, 4, false, !!this.hasBackpack_ ? 20 : 12);
                        if (_loc5_ != -1) {
                            this.map_.gs_.gsc_.invSwap(this, this, _loc5_, -1, param1, param2, param1.equipment_[param2]);
                        }
                    }
                } else {
                    this.map_.gs_.gsc_.invSwapRaw(this.x_, this.y_, param1.objectId_, param2, 2594, this.objectId_, 1000000, -1);
                }
            } else if (param3 == 2595) {
                if (this.magicPotionCount_ == 6) {
                    ...
                    this.map_.gs_.gsc_.invSwapRaw(this.x_, this.y_, param1.objectId_, param2, 2595, this.objectId_, 1000001, -1);
                }
            } else {
                _loc5_ = findItem(this.equipment_, -1, 4, false, !!this.hasBackpack_ ? 20 : 12);
                if (_loc5_ != -1) {
                    this.map_.gs_.gsc_.invSwap(this, this, _loc5_, this.equipment_[_loc5_], param1, param2, param1.equipment_[param2]);
                }
            }
        }
        ...
        public function checkHealth(param1: int = -1): Boolean {
            var _loc4_: * = undefined;
            ...
            if (!this.map_.gs_.isSafeMap) {
                ...
                if (!Parameters.data.fameBlockThirsty && !this.isSick && this.autoHpPotNumber != 0 && (this.hp_ <= this.autoHpPotNumber || this.clientHp <= this.autoHpPotNumber || this.syncedChp <= this.autoHpPotNumber) && param1 - this.lastHpPotTime > Parameters.data.autohpPotDelay) {
                    _loc5_ = false;
                    _loc6_ = !!this.hasBackpack_ ? 20 : 12;
                    _loc2_ = 4;
                    while (_loc2_ < _loc6_) {
                        _loc3_ = this.equipment_[_loc2_];
                        _loc4_ = new <int>[2736, 16874, 1799, 2594, 2868, 2870, 2872, 2874, 2876, 2836, 2837, 2838, 2839, 2689, 2632, 2633, 2795, 1799, 3105, 3090, 3164, 3163, 3265, 9077, 10244, 10243, 28901];
                        if (_loc4_.indexOf(_loc3_) != -1) {
                            if (param1 == -1) {
                                param1 = TimeUtil.getModdedTime();
                            }
                            this.map_.gs_.gsc_.useItem(param1, this.objectId_, _loc2_, _loc3_, this.x_, this.y_, 1);
                            _loc5_ = true;
                            break;
                        }
                        _loc2_++;
                    }
                    if (!_loc5_ && this.healthPotionCount_ > 0) {
        ...
        public function checkMana(param1: int = -1): void {
            ...
            if (!this.map_.gs_.isSafeMap) {
                ...
                if (this.autoMpPotNumber == 0 || Parameters.data.fameBlockThirsty || this.isQuiet_() || param1 - lastMpPotTime < Parameters.data.autompPotDelay) {
                    return;
                }
        ...
        public function sbAssist(param1: int, param2: int): void {
            var _loc10_: Number = NaN;
            var _loc6_: * = null;
            var _loc8_: int = this.equipment_[1];
            if (_loc8_ == -1) {
                return;
            }
            var _loc9_: XML = ObjectLibrary.xmlLibrary_[_loc8_];
            var _loc12_: int = 0;
            var _loc11_: * = _loc9_.Activate;
            for each(var _loc3_ in _loc9_.Activate) {
                if (_loc3_.toString() == "Teleport") {
                    this.useAltWeapon(param1, param2, 1, -1, false);
                    return;
                }
            }
            var _loc7_: Point = sToW(param1, param2);
            var _loc4_: * = Infinity;
            var _loc14_: int = 0;
        ...
        private function lookForMpPotAndDrink(param1: int): void {
            ...
            if (!_loc4_ && this.magicPotionCount_ > 0) {
        ...
        private function doShoot(param1: int, param2: int, param3: XML, param4: Number, param5: Boolean, param6: Boolean, param7: Boolean = false): void {
            ...
            var _loc15_: Number = ("ArcGap" in param3 ? Number(param3.ArcGap) : 11.25) * 0.0174532925199433;
            var _loc10_: Number = _loc15_ * (_loc13_ - 1);
            var _loc9_: Number = param4 - _loc10_ * 0.5;
            ...
            while (_loc17_ < _loc13_) {
                ...
                _loc8_ = _loc18_.projProps_.minDamage_;
                _loc20_ = _loc18_.projProps_.maxDamage_;
