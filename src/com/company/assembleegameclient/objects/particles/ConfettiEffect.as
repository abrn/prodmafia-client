...
public class ConfettiEffect extends ParticleEffect {
      ...
      public const colors:Array = [0xffff,0xff00ff,0xffff00,0xffffff];
      ...
      override public function update(param1:int, param2:int) : Boolean {
         var _local4:int = 0;
         var _local6:ConfettiParticle = null;
         x_ = this.start_.x;
         y_ = this.start_.y;
         var _local3:int = 5;
         var _local5:int = 0;
         while(_local5 < _local3) {
            if(this.confettiColor_) {
               _local4 = colors[int(Math.random() * 5)];
            } else {
               _local4 = this.color_;
            }
            _local6 = new ConfettiParticle(1.85,(3 + int(Math.random() * 5)) * 20,_local4,500 * this.lifetime_ + Math.random() * 500 * this.lifetime_,0.1 + Math.random() * 0.1,this.start_,this.end_);
