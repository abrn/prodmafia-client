...
public class SoundEffectLibrary {
    ...
    private static function getUrlBase():String {
        ...
        var _local2:String = "";
        try {
            _local1 = StaticInjectorContext.getInjector().getInstance(ApplicationSetup);
            _local2 = _local1.getAppEngineUrl(true);
        } catch (error:Error) {
            _local2 = "localhost";
        }
        return _local2;
    ...
    private static function makeSoundRequest(_arg_1:String):URLRequest {
        ...
        var _local2:String = "{URLBASE}/sfx/{NAME}.mp3".replace("{URLBASE}", urlBase).replace("{NAME}", _arg_1);
