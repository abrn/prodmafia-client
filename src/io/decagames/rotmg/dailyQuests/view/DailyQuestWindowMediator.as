package io.decagames.rotmg.dailyQuests.view {
    ...
    public class DailyQuestWindowMediator extends Mediator {
        ...
        private function setToolTipTitle(_arg_1: String, _arg_2: String): void {
            this.toolTip = new TextToolTip(0x363636, 0x9b9b9b, _arg_1, _arg_2, 5 * 60, null);
        ...
        private function buyRefreshToolTip(): IconToolTip {
            ...
            return new IconToolTip("Refresh Daily Quests", _local1, 0x363636, 1, 0x9b9b9b, 1, true, new Bitmap(IconFactory.makeCoin()));
        }
        ...
        private function onRedeemComplete(_arg_1: QuestRedeemResponse): void {
            ...
            try {
                if (_local3.ok) {
                    _local4 = this.dailyQuestsModel.currentQuest.id;
                    this.redeemPopup = new DailyQuestRedeemPopup(this.dailyQuestsModel.getQuestById(_local4), this.dailyQuestsModel.selectedItem);
                    this.dailyQuestsModel.markAsCompleted(this.dailyQuestsModel.currentQuest.id);
                    _local2 = this.dailyQuestsModel.numberOfCompletedQuests;
                    if (!this.dailyQuestsModel.currentQuest.repeatable) {
                        this.dailyQuestsModel.currentQuest.completed = true;
                    }
                    _local2++;
                    this.view.hideFade();
                    this.view.showFade(0x151515, _local2 == this.dailyQuestsModel.numberOfActiveQuests);
                    this.showPopupSignal.dispatch(this.redeemPopup);
                } else {
                    this.view.hideFade();
                    this.showPopupSignal.dispatch(new ErrorModal(5 * 60, "Quest Error", _local3.message));
                ...
                view.hideFade();
                showPopupSignal.dispatch(new ErrorModal(5 * 60, "Quest Error", error.message));
                onRedeemClose();
