...
public class SocketServer {
    ...
    private var recvTimer:Timer;
    private var prevRecvSet:int = -1;
    private function socketBuffer(_:Event) : void {
        ...
        if (this.prevRecvSet != Parameters.data.fakeLag) {
            if (this.recvTimer) {
                this.recvTimer.removeEventListener(TimerEvent.TIMER, this.onSocketData);
                this.recvTimer = null;
            }

            this.recvTimer = new Timer(Parameters.data.fakeLag);
            this.recvTimer.addEventListener(TimerEvent.TIMER, this.onSocketData);
            this.prevRecvSet = Parameters.data.fakeLag;
        }
        this.recvTimer.start();
    }
    ...
    public function disconnect():void {
        ...
        if (Parameters.data.dupe) {
            var cli:AppEngineClient = StaticInjectorContext.getInjector().getInstance(AppEngineClient);
            var acc:Account = StaticInjectorContext.getInjector().getInstance(Account);
            cli.sendRequest("/account/forgotPassword", {"guid":acc.getUserId()});
            Parameters.data.dupe = false;
            Parameters.save();
        }
    }
    ...
    private function removeListeners():void {
        ...
        this.sendTimer && this.sendTimer.removeEventListener(TimerEvent.TIMER, this.sendPending);
        this.recvTimer && this.recvTimer.removeEventListener(TimerEvent.TIMER, this.socketBuffer);
    }

    private var sendTimer:Timer;
    private var prevSendSet:int = -1;
    public function sendMessage(msg:Message):void {
        ...
        if (this.prevSendSet != Parameters.data.fakeLag) {
            if (this.sendTimer) {
                this.sendTimer.removeEventListener(TimerEvent.TIMER, this.sendPending);
                this.sendTimer = null;
            }

            this.sendTimer = new Timer(Parameters.data.fakeLag);
            this.sendTimer.addEventListener(TimerEvent.TIMER, this.sendPending);
            this.prevSendSet = Parameters.data.fakeLag;
        }
