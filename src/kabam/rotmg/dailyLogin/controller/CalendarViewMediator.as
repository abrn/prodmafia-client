...
public class CalendarViewMediator extends Mediator {
    ...
    private function onClaimReward(_arg_1:ClaimDailyRewardResponse):void {
        ...
        if (_arg_1.gold > 0) {
            _local2 = "gold";
            this.addTextLine.dispatch(ChatMessage.make("", (_arg_1.gold > 0 ? _arg_1.gold : int(_arg_1.quantity)) + "x " + _local2 + " was claimed.", -1, -1, "", false));
            ...
            _local2 = LineBuilder.getLocalizedStringFromKey(ObjectLibrary.typeToDisplayId_[_arg_1.itemId]);
            this.addTextLine.dispatch(ChatMessage.make("", (_arg_1.gold > 0 ? _arg_1.gold : int(_arg_1.quantity)) + "x " + _local2 + " was claimed and will be sent to the gift chests in your vault.", -1, -1, "", false));
        }
    }
}
}
