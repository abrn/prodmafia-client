...
public class Graphic3DHelper {
    ...
    private static function injectSingletonIndexBuffer(_arg_1:Injector):void {
        var _local2:IndexBufferFactory = _arg_1.getInstance(IndexBufferFactory);
        _arg_1.map(IndexBuffer3DProxy).toProvider(_local2);
    }

    private static function injectSingletonVertexBuffer(_arg_1:Injector):void {
        var _local2:VertexBufferFactory = _arg_1.getInstance(VertexBufferFactory);
        _arg_1.map(VertexBuffer3DProxy).toProvider(_local2);
    }
